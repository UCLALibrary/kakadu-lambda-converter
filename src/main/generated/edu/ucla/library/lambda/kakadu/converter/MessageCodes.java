package edu.ucla.library.lambda.kakadu.converter;
/**
 * BEGIN GENERATED CODE
 */
final public class MessageCodes {

	/**
	 * Message: Supplied compression rate is not a float value: {}
	 */
	public static final String LKC_119 = "LKC-119";
	/**
	 * Message: Unable to report successful image conversion to monitoring endpoint: {}
	 */
	public static final String LKC_118 = "LKC-118";
	/**
	 * Message: Successfully reported image conversion to monitoring endpoint: {}
	 */
	public static final String LKC_117 = "LKC-117";
	/**
	 * Message: Unable to create a temporary source file: {}
	 */
	public static final String LKC_116 = "LKC-116";
	/**
	 * Message: Unable to deliver update to JP2 conversion monitoring service: {} -> {}
	 */
	public static final String LKC_115 = "LKC-115";
	/**
	 * Message: Writing temporary TIFF file in Lambda function: {}
	 */
	public static final String LKC_114 = "LKC-114";
	/**
	 * Message: Conversion process returned an empty JP2: {}
	 */
	public static final String LKC_009 = "LKC-009";
	/**
	 * Message: Successfully uploaded '{}' to: s3://{}/{}
	 */
	public static final String LKC_008 = "LKC-008";
	/**
	 * Message: Unable to clean up Lambda function's temporary file: {}
	 */
	public static final String LKC_113 = "LKC-113";
	/**
	 * Message: File doesn't exist and can't be written: {}
	 */
	public static final String LKC_007 = "LKC-007";
	/**
	 * Message: Kakadu output: {}
	 */
	public static final String LKC_112 = "LKC-112";
	/**
	 * Message: Error converting image to JP2: {}
	 */
	public static final String LKC_111 = "LKC-111";
	/**
	 * Message: Failed to convert TIFF to JP2: {}
	 */
	public static final String LKC_006 = "LKC-006";
	/**
	 * Message: Byte array is {} bytes long
	 */
	public static final String LKC_005 = "LKC-005";
	/**
	 * Message: Retrieved object's metadata says it is {} bytes long; it is {} bytes long
	 */
	public static final String LKC_004 = "LKC-004";
	/**
	 * Message: Error getting object '{}' from bucket '{}' (check that object exists and that bucket is in the
	    same region as this lambda function)
	 */
	public static final String LKC_003 = "LKC-003";
	/**
	 * Message: New S3 object received: {}/{} [{}]
	 */
	public static final String LKC_002 = "LKC-002";
	/**
	 * Message: Received S3 event: {}
	 */
	public static final String LKC_001 = "LKC-001";
	/**
	 * Message: {}
	 */
	public static final String LKC_000 = "LKC-000";
	/**
	 * Message: S3 object '{}' could not be written to the file system: {}
	 */
	public static final String LKC_010 = "LKC-010";

	private MessageCodes() {
		super();
	}
}